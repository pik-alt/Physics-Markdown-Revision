...
$\frac{1}{3}Nm \bar{c^{2}} = NkT$

mean gas kinetic energy is given by $\bar{E_{k}} = \frac{1}{2}m \bar{c^{2}}$ (just standard $E_{k}$ formula but replace $v^{2}$)
...
---
For an ideal gas, since interactions are negligible, total potential energy is zero (for some reason)
